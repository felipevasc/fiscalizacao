...
const DetalheOS = () => {
  ...
  const ItemInfo: React.FC<{ rotulo: string; valor?: string | null }> = ({
    rotulo,
    valor,
  }) => (
    <Grid item xs={12} sm={6} md={4}>
      <Typography
        variant='body2'
        component='div'
        sx={{ color: '#5E6E80', mb: 0.5, fontSize: '0.875rem' }}>
        {rotulo}:
      </Typography>
      <Typography
        variant='subtitle1'
        component='div'
        sx={{ fontWeight: 500, color: '#212529', wordBreak: 'break-word' }}>
        {valor || (
  ...
  return (
    <Box
      ...
      <Paper sx={estiloCardSecao} elevation={0}>
        <Typography sx={estiloTituloSecao} component='h2'>
          Prioridade (Matriz GUT)
        </Typography>
        <Grid container spacing={3}>
          <ItemInfo rotulo='Gravidade' valor={os.gravidade?.toString()} />
          <ItemInfo rotulo='Urgência' valor={os.urgencia?.toString()} />
          <ItemInfo rotulo='Tendência' valor={os.tendencia?.toString()} />
          <ItemInfo rotulo='Score GUT' valor={os.gutScore?.toString()} />
        </Grid>
        <Box sx={{ mt: 2, mb: 1, display: 'flex', alignItems: 'center' }}>
          <Typography
            variant='body2'
            sx={{ mr: 1, fontWeight: 500, color: '#333' }}>
            Prioridade:
          </Typography>
          <Box
            sx={{
          ...
          <Typography
            ...
            sx={{
              ml: 1,
              fontWeight: 600,
              color: corPorPrioridade(os.gutScore),
            }}>
            {rotuloPorPrioridade(os.gutScore)}
          </Typography>
        </Box>
      </Paper>
      ...
      <Paper sx={estiloCardSecao} elevation={0}>
        ...
        {itens.length === 0 ? (
          <Typography sx={{ color: '#555555', fontStyle: 'italic' }}>
            (Nenhum item cadastrado para esta OS)
          </Typography>
        ) : (
          <Box sx={{ overflowX: 'auto' }}>
            <Table
              size='small'
              aria-label='Itens da Ordem de Serviço'
              sx={{
      ...
      <Paper sx={estiloCardSecao} elevation={0}>
        ...
        {instrucoes ? (
          ...
          <Typography sx={{ color: '#555555', fontStyle: 'italic' }}>
            (Nenhuma instrução complementar fornecida)
          </Typography>
        )}
      </Paper>
      ...
      <Paper sx={estiloCardSecao} elevation={0}>
        ...
        <Grid container spacing={3} sx={{ mb: 3 }}>
          <ItemInfo
            rotulo='Data de Início Prevista'
            valor={cronograma.data_inicio}
          />
